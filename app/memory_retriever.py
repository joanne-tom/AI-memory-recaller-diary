...
def migrate_memories(user_id):
    ...
    for memory in memories:
        ...
        emotion = classify_text(memory_text)
        emotion_value=emotion['label']
...
def chat_with_user(user_id, user_input, happy_memory_provided=False):
    ...
    if emotion_value in ["sadness", "anger", "pessimism", "fear", "disgust"]:
        memory = retrieve_memory(user_id, emotion_value, user_input)
        print('retrieved:',memory)
        prompt = f'The user is feeling {emotion_value}. Respond with a comforting and empathetic message and gently remind them,using second-person language (like "you" or "your") — do NOT imply the chatbot was present,of this memory: {memory}. Say that you hope this memory brings some comfort, and end the message with a kind goodbye.'
        response = model.generate_content(prompt)
        ...
        if happy_memory_provided:
            ...
            prompt = f"The user is feeling {emotion_value}. Respond with a short, encouraging message and ask a line about their happy moment."
            response = model.generate_content(prompt)
